...
|-------|-----------------|--------------------------------|
| **Customer / anonymous visitor** | Availability, headline, curated photos, specs (bed/bath/type), amenity tags, price + cadence (`/mo`, `/sale`), **broad geography** (`county`, `town`, optional fuzzy `area_label` like “Northern Kiambu” **without street/estate/pin**) | Exact `latitude/longitude`, `estate`, `address`, landlord/agent **phone**, internal notes, draft listings |
| **Admin (authenticated, role=admin)** | Full property row, coordinates, map markers, contact phone(s), internal fields, upload paths, AI drafts | N/A (subject to future super‑admin splits) |
...
**`saved_properties`** (future customer accounts)
- For v1 can be disabled or gated behind auth.
...
- **`properties`**
  - `SELECT` **anon + authenticated**: rows where `is_published = true AND is_available = true` **returning only safe columns** → implement via **`properties_public` VIEW** (see below) *or* duplicate with edge function. **Preferred v1:** view + grants.
...
**View `properties_public` columns:**  
`id, title, description, ai_generated_description, price, price_type, bedrooms, bathrooms, property_type, county, town, area_label, is_available, listing_reference, cover_image_url, created_at`  
*No lat/lng/estate/address/owner_phone.*

Admin app uses direct table or `properties_full` view including secrets.
...
### 4.1 Public site (`/`)
- **Landing:** bold hero, value props (“See what’s available — we’ll place you”), search by county/town/type/price.
- **Browse:** responsive card grid, filters, sort (price, recency).
- **Listing detail:** photo carousel, specs, amenities, **broad location line only**, **no map embed**. Primary CTA: **Call / WhatsApp HomeTrace**. Secondary: “Request viewing” form (future).
- **About / Trust / Legal** stub pages optional.

### 4.2 Admin (`/admin/*`)
- **Login** (Supabase email magic link or password — v1: magic link recommended).
- **Dashboard:** counts, quick add.
- **Map:** Leaflet full screen split with list; markers from `properties` where coords present.
- **Add / Edit property:** form with geocode helper (lat/lng paste + link to Google Maps), image multi‑upload to storage, amenity chips, AI description button (Gemini), publish toggle.
- **Unpublished queue:** table of drafts.
...
- [ ] Admin routes gated by auth + `role === 'admin'` (client) **and** enforced by RLS (server).
- [ ] Storage policies: only admin can upload.
- [ ] Validate image MIME types & size limits client‑side; add server policy when possible.
...
- [ ] Customer auth + saved favourites
- [ ] SEO meta + OG images
